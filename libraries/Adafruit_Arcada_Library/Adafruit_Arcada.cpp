...
/**************************************************************************/
/*! // GROMD --BEGIN-- \\
    @brief  Set the gif play time
    @param  time From 0 (off) to infinity (86400) (1 day)
    @param  saveToDisk Whether we save this permanently to disk, default is
   false
    @returns Whether saving to disk succeeded, or true if we don't save
*/
/**************************************************************************/
bool Adafruit_Arcada_SPITFT::setGIFtime(uint8_t seconds_per_gif, bool saveToDisk) {
	_seconds_per_gif = seconds_per_gif;
#ifdef ARCADA_USE_JSON
  configJSON["seconds_per_gif"] = _seconds_per_gif;
  if (saveToDisk) {
	  return saveConfigurationFile(); 
  }
#endif
...
/**************************************************************************/
uint8_t Adafruit_Arcada_SPITFT::getGIFtime(void) { return _seconds_per_gif; }
/**************************************************************************/
/*! // GRMOD -RANDOM- \\
    @brief  Set random on or off
    @param  0 == off | 1 == on
    @param  saveToDisk Whether we save this permanently to disk, default is
   false
    @returns Whether saving to disk succeeded, or true if we don't save
*/
/**************************************************************************/
bool Adafruit_Arcada_SPITFT::setRandom(uint8_t y, bool saveToDisk) {
	_random = y;
#ifdef ARCADA_USE_JSON
  configJSON["random"] = _random;
  if (saveToDisk) {
	  return saveConfigurationFile(); 
  }
#endif
...
/**************************************************************************/
uint8_t Adafruit_Arcada_SPITFT::getRandom(void) { return _random; }
/**************************************************************************/
/*!
